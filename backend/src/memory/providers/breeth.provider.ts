...
export class BreethMemoryProvider {
  ...
  async recordEpisode(episode: BreethMemoryEpisode): Promise<string | null> {
    ...
    try {
      ...
      logger.warn({ err: err.message }, 'Failed to record episode to Breeth API, continuing cycle.');
      return null;
    }
  }
  ...
  async searchMemory(query: string, limit: number = 5): Promise<string[]> {
    ...
    try {
      ...
      logger.warn({ err: err.message }, 'Breeth memory search timed out/failed, returning fallback local memory.');
      return [];
    }
  }
}
